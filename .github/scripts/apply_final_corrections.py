...
for path in program_files:
    ...
    # A prior QA attempt placed this styling in the imported component's own style block.
    # dc-import does not carry that top-level style into the rendered page, so remove it
    # from component copies and keep the live styling in rendered-corrections.css instead.
    if 'SUNDAY ARCHIVE LIBRARY CARD MODALS' in text:
        start = text.find('  /* SUNDAY ARCHIVE LIBRARY CARD MODALS */')
        if start == -1:
            start = text.find('/* SUNDAY ARCHIVE LIBRARY CARD MODALS */')
...
if 'SUNDAY ARCHIVE LIBRARY CARD MODALS' not in css:
    css = css.rstrip() + '\n\n' + LIBRARY_CSS + '\n'
...
print(f'Updated {len(program_files)} Program Cards copies, global archival modal styling, and v8 assets')
